...
import { SubmitButtonComponent } from '../../../shared/components/submitButton/submit-button.component';
    ...
    this.wishlistService
      ...
      .pipe(
        switchMap((response) => {
          ...
          const currentQuantity = serverProducts.length;
          ...
          return requests.length > 0
            ? forkJoin(requests).pipe(
                switchMap((responses) =>
                  of({
                    currentQuantity: responses.at(-1)?.data.length ?? currentQuantity,
                  }),
                ),
              )
            : of({ currentQuantity });
